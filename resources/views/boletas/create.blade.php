...
@section('title', 'Crear Boleta para ' . $estudiante->nombres . ' ' . $estudiante->apellidos)

@section('content')
<div class="container">
    <h2>
        <i class="fas fa-clipboard-list m-2"></i>
        Crear Boleta para {{ $estudiante->nombres }} {{ $estudiante->apellidos }}
    </h2>
    ...
    <div class="card">
        <div class="card-body">
            ...
            <form action="{{ route('boletas.store', $estudiante) }}" method="POST">
                @csrf
                ...
                <div class="mb-3">
                    ...
                    <select name="ano_escolar_id" id="ano_escolar_id" class="form-select" required>
                        ...
                        @foreach($anosEscolares as $anoEscolar)
                            <option value="{{ $anoEscolar->id }}" {{ old('ano_escolar_id') == $anoEscolar->id ? 'selected' : '' }}>
                                {{ $anoEscolar->nombre }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select name="momento" id="momento" class="form-select" required>
                        <option value="1er Momento" {{ old('momento') == '1er Momento' ? 'selected' : '' }}>1er Momento</option>
                        <option value="2do Momento" {{ old('momento') == '2do Momento' ? 'selected' : '' }}>2do Momento</option>
                        <option value="3er Momento" {{ old('momento') == '3er Momento' ? 'selected' : '' }}>3er Momento</option>
                    </select>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select name="tipo_boleta" id="tipo_boleta" class="form-select" required>
                        <option value="descriptiva" {{ old('tipo_boleta') == 'descriptiva' ? 'selected' : '' }}>Descriptiva</option>
                        <option value="calificativa" {{ old('tipo_boleta') == 'calificativa' ? 'selected' : '' }}>Calificativa</option>
                    </select>
                </div>
                ...
                <div id="sistema_calificacion_container" class="mb-3" style="display: none;">
                    ...
                    <select name="sistema_calificacion" id="sistema_calificacion" class="form-select">
                        <option value="literal" {{ old('sistema_calificacion') == 'literal' ? 'selected' : '' }}>Literal (A, B, C, D, E)</option>
                        <option value="numerica" {{ old('sistema_calificacion') == 'numerica' ? 'selected' : '' }}>Numérica (0-20)</option>
                    </select>
                </div>

                <div class="mb-3">
                    <label for="proyecto" class="form-label">Nombre del Proyecto</label>
                    <input type="text" name="proyecto" id="proyecto" class="form-control" value="{{ old('proyecto') }}" required>
                </div>
                ...
                <div id="calificacion_general_container" class="mb-3" style="display: none;">
                    ...
                    <div id="calificacion_general_literal">
                        <select name="calificacion_general" id="calificacion_general_literal_select" class="form-select">
                            ...
                            <option value="A" {{ old('calificacion_general') == 'A' ? 'selected' : '' }}>A</option>
                            <option value="B" {{ old('calificacion_general') == 'B' ? 'selected' : '' }}>B</option>
                            <option value="C" {{ old('calificacion_general') == 'C' ? 'selected' : '' }}>C</option>
                            <option value="D" {{ old('calificacion_general') == 'D' ? 'selected' : '' }}>D</option>
                            <option value="E" {{ old('calificacion_general') == 'E' ? 'selected' : '' }}>E</option>
                        </select>
                    </div>
                    
                    <!-- Calificación general numérica -->
                    <div id="calificacion_general_numerica" style="display: none;">
                        <input type="number" name="calificacion_general_numerica" id="calificacion_general_numerica_input" class="form-control" min="0" max="20" step="1" value="{{ old('calificacion_general_numerica') }}">
                    </div>
                </div>
                ...
                <div class="mb-3 asignatura-container">
                    ...
                    <div class="descripcion-field">
                        ...
                        <textarea name="calificaciones[{{ $asignatura->id }}][descripcion]" class="form-control" rows="3">{{ old("calificaciones.{$asignatura->id}.descripcion") }}</textarea>
                    </div>
                    ...
                    <div class="calificacion-literal-field" style="display: none;">
                        ...
                        <select name="calificaciones[{{ $asignatura->id }}][calificacion_literal]" class="form-select">
                            <option value="">Seleccione una calificación</option>
                            <option value="A" {{ old("calificaciones.{$asignatura->id}.calificacion_literal") == 'A' ? 'selected' : '' }}>A</option>
                            <option value="B" {{ old("calificaciones.{$asignatura->id}.calificacion_literal") == 'B' ? 'selected' : '' }}>B</option>
                            <option value="C" {{ old("calificaciones.{$asignatura->id}.calificacion_literal") == 'C' ? 'selected' : '' }}>C</option>
                            <option value="D" {{ old("calificaciones.{$asignatura->id}.calificacion_literal") == 'D' ? 'selected' : '' }}>D</option>
                            <option value="E" {{ old("calificaciones.{$asignatura->id}.calificacion_literal") == 'E' ? 'selected' : '' }}>E</option>
                        </select>
                    </div>
                    ...
                    <div class="calificacion-numerica-field" style="display: none;">
                        ...
                        <input type="number" name="calificaciones[{{ $asignatura->id }}][calificacion_numerica]" class="form-control" min="0" max="20" step="1" value="{{ old("calificaciones.{$asignatura->id}.calificacion_numerica") }}">
                    </div>
                </div>
                @endforeach
